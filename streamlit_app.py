# Streamlit app for Churn Prediction using best_optuna_churn_model.pkl
...
# Utility functions
@st.cache_data
def load_data(path='Customer-Churn (1).csv'):
    df = pd.read_csv(path)
    return df
...
@st.cache_resource
def load_model(path='best_optuna_churn_model.pkl'):
